...
{
	// Format output based on whether real part is whole number and single digit
    double real = output.real;
	// Check if real part is whole number and single digit
    bool isWhole = (real == static_cast<int>(real));
    bool isSingleDigit = (real >= 0 && real < 10);
	// Output formatted complex number
    if (isWhole && isSingleDigit)
        outstream << "( " << real << ", " << output.imaginary << ')';
    else
